...
% --- Reset section counter về 0 mỗi khi bắt đầu chapter mới ---
\makeatletter
...
\newcommand{\monan}[6]{
    ...
    \noindent
    {\Large\bfseries\color{brown!80!black}\thesection. #1}
    \addcontentsline{toc}{section}{\protect\numberline{\thesection}#1}
    \index{#1} % Thêm món ăn vào chỉ mục

    % Hiển thị độ khó và thời gian ngay sau tên món
    \hfill
    \textcolor{brown!70!black}{
        \textbf{Độ khó:}
        \ifcase#2
        \or $\star$ {\small\textit{(Dễ)}}
        \or $\star\star$ {\small\textit{(Trung bình)}}
        \or $\star\star\star$ {\small\textit{(Khó)}}
        \fi
        \quad $\bullet$ \quad
        \textbf{Thời gian:} #3
    }
